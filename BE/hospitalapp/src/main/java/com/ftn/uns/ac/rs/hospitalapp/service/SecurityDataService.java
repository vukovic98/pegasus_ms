...
public class SecurityDataService {
	...
	public boolean createRuleForLogType(LogTypeDTO logType) {
		...
		try {
			...
			FileOutputStream drlFile = new FileOutputStream(new File(
					"..\\security-kjar\\src\\main\\resources\\sbnz\\logs-"+logType.getLogType()+".drl"));
	...
	public boolean createRuleForDoSAttack(DoSDTO dosDto) {

		try {
			InputStream template = new FileInputStream(
					"..\\security-kjar\\src\\main\\resources\\sbnz\\templates\\dos-template.drt");

			// Compile template to generate new rules
			List<DoSDTO> arguments = new ArrayList<>();
			arguments.add(dosDto);
			...
			// Save rule to drl file
			FileOutputStream drlFile = new FileOutputStream(new File(
					"..\\security-kjar\\src\\main\\resources\\sbnz\\dos-"+dosDto.getIpAddress().replace(':','-').replace('.', '-')+"-"+dosDto.getNoRequests()+".drl"));
			drlFile.write(drl.getBytes());
			drlFile.close();
